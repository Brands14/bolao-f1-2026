...
def main():
    ...
    if menu == "Enviar Palpite":
        ...
        if usuario_logado:
            ...
            # --- TRAVA DE SEGURANÇA: FILTRO DE DATAS ---
            fuso_sp = pytz.timezone("America/Sao_Paulo")
            hoje = datetime.now(fuso_sp).date()
            
            # Só coloca na lista se a data do GP for MAIOR ou IGUAL a hoje
            ...
            if restante < 1800:
                st.error(f"❌ Prazo encerrado para o {gp_escolhido}!")
            else:
        ...
        if not df_p.empty and not df_g.empty:
            ...
            for _, g in df_g.iterrows():
                ...
                for _, palpite in p_gp.iterrows():
                    ...
                    resultados.append({"Usuario": palpite['Usuario'], "Equipe": palpite['Equipe'], "Pontos": pts})
            
            if resultados:
                df_res = pd.DataFrame(resultados)
                st.subheader("Individual")
                st.dataframe(df_res.groupby("Usuario")["Pontos"].sum().sort_values(ascending=False))
        else:
            st.warning("Aguardando resultados...")

    elif menu == "⚙️ Admin":
        senha = st.text_input("Senha Admin:", type="password")
        if senha == "f12026admin":
            st.header("Inserir Gabarito")
            with st.form("gabarito"):
                g_gp = st.selectbox("GP:", lista_gps)
                g_tipo = st.selectbox("Sessão:", ["Classificação Principal (Pole)", "Corrida Principal"])
