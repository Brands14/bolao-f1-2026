...
# --- FUNÇÃO DE EXIBIÇÃO DE FOTOS (MELHORIA ADICIONADA) ---
...
# --- LISTAS E DICIONÁRIOS (RESTAURADOS DO TXT) ---
...
# 2. Motor GitHub e Funções (RESTAURADOS TOTALMENTE DO TXT)
...
def enviar_recibo_email(dados, email_destino):
    ...
    msg = MIMEMultipart()
    msg['From'] = remetente; msg['To'] = ", ".join(destinatarios)
    msg['Subject'] = f"🏁 Recibo de Palpite F1 - {dados['Usuario']} - GP {dados['GP']}"
    corpo = f"Olá {dados['Usuario']}, palpite recebido para {dados['GP']} ({dados['Tipo']})."
...
def calcular_pontos_sessao(palpite, gabarito):
    pontos = 0; tipo = palpite.get('Tipo', '')
    if "Pole" in tipo:
...
if menu == "Enviar Palpite":
    ...
    if usuario_logado:
        ...
        c1, c2 = st.columns(2)
        with c1: gp_sel = st.selectbox("Grande Prêmio:", lista_gps)
        with c2: tipo_sel = st.selectbox("Sessão:", ["Classificação Principal (Pole)", "Corrida Principal", "Qualy Sprint (Pole)", "Corrida Sprint"] if gp_sel in sprint_gps else ["Classificação Principal (Pole)", "Corrida Principal"])
        ...
        with st.form("form_envio"):
            email_confirm = st.text_input("E-mail cadastrado:", type="password")
            if st.form_submit_button("GRAVAR PALPITE 🏁"):
                if email_confirm.lower() == emails_autorizados.get(usuario_logado, "").lower():
                    ...
                    if guardar_dados(dados, ARQUIVO_DADOS):
                        enviar_recibo_email(dados, email_confirm); st.success("Sucesso!")
                    else: st.error("Erro no servidor.")
                else: st.error("E-mail incorreto.")

# 4. MEUS PALPITES (RESTAURAÇÃO TOTAL DO TXT)
    ...
    if usuario_logado:
        ...
        if not df.empty:
            ...
            if not meus_votos.empty: st.dataframe(meus_votos)
            else: st.info("Sem palpites.")
        else: st.info("Banco vazio.")

# 5. CLASSIFICAÇÕES (RESTAURAÇÃO TOTAL DO TXT - GERAL E ETAPA)
    ...
    if not df_palpites.empty and not df_gabaritos.empty:
        ...
        for _, palpite in df_palpites.iterrows():
            ...
            if not gabarito.empty:
                ...
                pontuacoes.append({"GP": palpite['GP'], "Usuario": palpite['Usuario'], "Equipe": palpite['Equipe'], "Pontos": pts})
        
        if pontuacoes:
            df_pts = pd.DataFrame(pontuacoes)
            rank = df_pts.groupby(['Usuario', 'Equipe']).sum().sort_values(by="Pontos", ascending=False).reset_index()
            st.table(rank)
            st.header("📍 Por Etapa (Usuários)")
            gp_etapa = st.selectbox("Selecione o GP:", lista_gps)
            st.table(df_pts[df_pts['GP'] == gp_etapa].groupby('Usuario').sum().sort_values(by="Pontos", ascending=False).reset_index())
    else: st.info("Aguardando resultados.")

# 6. ADMINISTRADOR (RESTAURAÇÃO TOTAL DO TXT)
elif menu == "Administrador":
    st.header("🔐 Painel do Comissário")
    senha_adm = st.text_input("Senha de Acesso:", type="password")
    if senha_adm == "f12026":
        st.success("Acesso Liberado!")
        aba1, aba2 = st.tabs(["Lançar Resultados", "Limpeza"])
        with aba1:
            gp_res = st.selectbox("GP:", lista_gps)
            tipo_res = st.selectbox("Sessão:", ["Classificação Principal (Pole)", "Corrida Principal", "Qualy Sprint (Pole)", "Corrida Sprint"])
            with st.form("form_gabarito"):
                g_pole = st.selectbox("Pole Oficial:", pilotos)
                if st.form_submit_button("SALVAR"):
                    if guardar_dados({"GP": gp_res, "Tipo": tipo_res, "Pole": g_pole}, ARQUIVO_GABARITOS): st.success("Salvo!")
        with aba2:
            df_limpa, _ = ler_dados(ARQUIVO_DADOS)
            if not df_limpa.empty:
                u_del = st.selectbox("Usuário para apagar:", df_limpa['Usuario'].unique())
                if st.button("CONFIRMAR EXCLUSÃO"):
                    if deletar_registro_github(ARQUIVO_DADOS, df_limpa['Usuario'] != u_del): st.success("Apagado!"); st.rerun()
